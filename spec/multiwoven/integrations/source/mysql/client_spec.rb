...
RSpec.describe Multiwoven::Integrations::Source::Mysql::Client do # rubocop:disable Metrics/BlockLength
  ...
  # read and #discover tests for Mysql
  describe "#read" do
    context "when reading records from a real Mysql database" do
...
RSpec.describe Multiwoven::Integrations::Source::Mysql::Client do
  ...
  describe "#check_connection" do
    ...
    context "when the connection fails" do
      it "returns a failed connection status with an error message" do
        ...
        allow(client).to receive(:create_connection).and_raise(Mysql2::Error.new("Connection failed"))
  ...
  describe "#discover" do
    it "discovers schema successfully" do
      ...
      allow(client).to receive(:query).and_return([
                                                    { "TABLE_NAME" => "table1", "COLUMN_NAME" => "column1", "DATA_TYPE" => "varchar", "IS_NULLABLE" => "YES" },
                                                    { "TABLE_NAME" => "table2", "COLUMN_NAME" => "column2", "DATA_TYPE" => "int", "IS_NULLABLE" => "NO" }
                                                  ])
      message = client.discover(connection_config)

      expect(message.catalog).to be_an(Multiwoven::Integrations::Protocol::Catalog)
      expect(message.catalog.streams.size).to eq(2)

      first_stream = message.catalog.streams[0]
      expect(first_stream).to be_a(Multiwoven::Integrations::Protocol::Stream)
      expect(first_stream.name).to eq("table1")
      expect(first_stream.json_schema).to eq({ "column1" => { "type" => %w[string null] } })

      second_stream = message.catalog.streams[1]
      expect(second_stream).to be_a(Multiwoven::Integrations::Protocol::Stream)
      expect(second_stream.name).to eq("table2")
      expect(second_stream.json_schema).to eq({ "column2" => { "type" => "int" } })
    end

    it "handles exception when discovering schema" do
      connection_config = {
        "credentials": {
          "auth_type": "username/password",
          "username": "test_username",
      ...
      allow(client).to receive(:create_connection)
      allow(client).to receive(:query).and_raise(StandardError.new("test error"))
      expect(client).to receive(:handle_exception).with(
        "MYSQL:DISCOVER:EXCEPTION",
        "error",
        an_instance_of(StandardError)
  ...
  describe "#read" do
    it "reads records successfully" do
      sync_config = double("sync_config")
      connection_config = {
        "credentials": {
          "auth_type": "username/password",
          "username": "test_username",
      ...
      allow(sync_config).to receive_message_chain("source.connection_specification").and_return(connection_config)
      allow(client).to receive(:create_connection)
      allow(client).to receive(:query).and_return([
                                                    { "column1" => "value1" },
                                                    { "column2" => "value2" }
      ...
      expect(records[0]).to be_a(Multiwoven::Integrations::Protocol::MultiwovenMessage)
      expect(records[0].data).to eq({ "column1" => "value1" })
      expect(records[1]).to be_a(Multiwoven::Integrations::Protocol::MultiwovenMessage)
      expect(records[1].data).to eq({ "column2" => "value2" })
    end

    it "handles exception when reading records" do
      sync_config = double("sync_config")
      connection_config = {
        "credentials": {
          "auth_type": "username/password",
