  ...
  module Integrations
    module Source
      module SalesforceConsumerGoodsCloud
        ...
        API_VERSION = "59.0"
        SALESFORCE_OBJECTS = %w[Account User]
        ...
        class Client < SourceConnector
          ...
          def sf_field_to_json_schema_type(sf_field)
            case sf_field['type']
            ...
            when 'picklist', 'multipicklist', 'Picklist (Multi-select)'
              if sf_field[:picklistValues] && sf_field['nillable']
                ...
                { "type": ["array", "null"], "items": { "type": "string" }, "enum": enum_values }
            ...
            when 'location', 'Geolocation'
              if sf_field['nillable']
                { "type": ["object", "null"], "properties": {"latitude": {"type": "number"}, "longitude": {"type": "number"}} }
              else
          ...
          def create_json_schema_for_object(metadata)
            fields_schema = metadata['fields'].map do |field|
              {
                "#{field[:name]}": sf_field_to_json_schema_type(field)
              }
            end.reduce(:merge)
